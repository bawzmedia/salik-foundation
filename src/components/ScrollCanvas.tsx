  ...
  function ScrollCanvas({ onProgressChange, onFallback }, ref) {
        ...
        return new Promise((resolve) => {
          ...
          const playNext = (now: number) => {
            ...
            if (done) {
              ...
              // Start idle ping-pong loop only after clip 1
              if (sectionIndex === 0) {
    ...
    useEffect(() => {
      ...
      const handleTouchEnd = (e: TouchEvent) => {
        ...
        // Mobile: vertical swipe (up/down) to navigate sections
        // Swipe up (diffY > 0) → next section, swipe down (diffY < 0) → previous
        if (Math.abs(diffY) > 50 && Math.abs(diffY) > Math.abs(diffX)) {
          if (diffY > 0) {
            // Swipe up → play next section
            const nextSection = currentSectionRef.current + 1;
            if (nextSection < SECTIONS.length) {
              scrollCooldownRef.current = true;
              playSection(nextSection);
            ...
            // Swipe down → play previous section in reverse
            const prevSection = currentSectionRef.current - 1;
            if (prevSection >= 0) {
              scrollCooldownRef.current = true;
              playSection(prevSection, true);
    ...
    // Flash sequence for "Take the Shot" button
      ...
      return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black">
          <div className="text-center text-white">
            <div className="mb-4 text-lg">Loading experience...</div>
        ...
        {ctaGate === "showing" && (
          ...
          >
            <button
              ...
              className="group relative overflow-hidden border border-white/40 bg-black/30 px-12 py-5 backdrop-blur-sm transition-all duration-500 hover:border-white/80 hover:bg-black/50"
            >
              <span className="relative font-cinzel text-xl tracking-[0.3em] text-white uppercase md:text-2xl">
                Start Your Hunt
              </span>
            </button>
          </div>
